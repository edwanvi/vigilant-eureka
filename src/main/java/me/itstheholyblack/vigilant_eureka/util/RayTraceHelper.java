...
/**
 * Class to help with raytracing. Copied from vanilla by Tamaized.
 *
 * @author Tamaized
 */
public class RayTraceHelper {
     ...
     * @return
     */
    public static RayTraceResult tracePath(World world, EntityPlayer player, int distance, float borderSize,
     ...
     * @return
     */
    public static RayTraceResult tracePath(World world, Vec3d vec1, Vec3d vec2, float borderSize,
                                           HashSet<Entity> excluded) {
        return tracePath(world, (float) vec1.x, (float) vec1.y, (float) vec1.z, (float) vec2.x, (float) vec2.y,
                (float) vec2.z, borderSize, excluded);
    }
     ...
     * @return
     */
    private static RayTraceResult tracePath(World world, float x, float y, float z, float tx, float ty, float tz,
        ...
        AxisAlignedBB bb = new AxisAlignedBB(minX, minY, minZ, maxX, maxY, maxZ).expand(borderSize, borderSize,
                borderSize);
        List<Entity> allEntities = excluded == null ? Lists.<Entity>newArrayList()
